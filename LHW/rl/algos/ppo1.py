...
"""
    ...
    def train(self, env_fn, n_itr, render: bool = False, render_every: int = 1):
        """
        Original train() preserved; added optional rendering.
        - render: if True, will run a short deterministic rollout using the current policy for visualization.
        - render_every: run the visualization every `render_every` training iterations (default: 1).
        """
        ...
        # --- Setup rendering environment (separate from sampling envs) ---
        render_env = None
        viewer = None
        if render:
            try:
                render_env = env_fn()
                # Try to launch mujoco viewer (Triang) if available
                try:
                    import mujoco
                    import mujoco.viewer
                    # If env exposes model and data, launch passive viewer
                    if hasattr(render_env, "model") and hasattr(render_env, "data"):
                        try:
                    ...
                    # mujoco or viewer not available; we'll call env.render() each step
                    viewer = None
                print("Visualization: render environment prepared.")
            except Exception as e:
                render_env = None
                viewer = None
                print(f"Visualization: failed to prepare render env: {e}")
        ...
        for itr in range(n_itr):
            ...
            # --- LIVE VISUALIZATION ROLLOUT (deterministic) ---
            if render and render_env is not None and ((itr % max(1, render_every)) == 0):
                try:
                    # deterministic rollout using current policy
                    print("[Visualization] Starting deterministic rollout for current policy...")
                    obs_v = render_env.reset()
                    done_v = False
                    steps_v = 0
                    max_v = min(self.max_traj_len, 1000)  # safety cap
                    # initialize hidden states if needed
                    if hasattr(self.policy, 'init_hidden_state'):
                        self.policy.init_hidden_state()
                    while (not done_v) and (steps_v < max_v):
                        with torch.no_grad():
                            state_t = torch.tensor(obs_v, dtype=torch.float)
                            action_v = self.policy(state_t, deterministic=True)
                            # If policy returns torch.Tensor, convert; handle shapes
                            if isinstance(action_v, torch.Tensor):
                                a_np = action_v.cpu().numpy()
                            else:
                                a_np = np.array(action_v)
                        next_obs_v, reward_v, done_v, info_v = render_env.step(a_np if hasattr(a_np, 'copy') else a_np)
                        # Show frame: use viewer if launched, otherwise fallback to env.render()
                        if viewer is not None:
                            try:
                                # If Triang viewer, sync to show new state
                                try:
                                    viewer.sync()
                                except Exception:
                                    # Fallback to calling env.render()
                                    try:
                                        render_env.render()
                                    except Exception:
                                        pass
                            ...
                            try:
                                ...
                                # ignore rendering errors (headless environment)
                                pass
                        obs_v = next_obs_v
                        steps_v += 1
                    print("[Visualization] Rollout finished ({} steps).".format(steps_v))
                except Exception as e:
                    print(f"[Visualization] Error during rollout: {e}")
